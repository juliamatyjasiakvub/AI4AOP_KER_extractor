...
def ensure_dump(timeout: int = 120) -> tuple[Optional[str], Optional[str]]:
    """
    Make sure a dump exists locally, fetching the newest one if none does.

    The dump used to ship inside the repository. It is roughly ten megabytes of
    somebody else's curated database, and redistributing it is a licensing
    decision rather than a packaging convenience — so the repository ships the
    code and the application fetches the data.

    Returns `(version, error)`. A None version with an error message means
    enrichment is unavailable, which is a degraded mode rather than a failure:
    `get_index()` already returns None when there is no dump, and extraction
    and curation run without AOP-Wiki identifiers exactly as they do when OLS4
    is unreachable.
    """
    ...
    remote = get_latest_remote_version(timeout=10)
    if remote is None:
        return None, (
            "No local AOP-Wiki dump, and aopwiki.org could not be reached. "
            "Extraction and curation will run without AOP-Wiki identifiers. "
            "To enable enrichment, download a dump from "
            f"https://aopwiki.org/downloads into {DATA_DIR}."
        )
    try:
        download_dump(remote, timeout=timeout)
